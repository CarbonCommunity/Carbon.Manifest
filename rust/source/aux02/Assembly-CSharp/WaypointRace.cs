...
public class WaypointRace : BaseEntity
{
	...
	public static void startRace (ConsoleSystem.Arg arg)
	{
		...
		List<BaseVehicle> obj3 = Pool.GetList<BaseVehicle> ();
	...
	private void Update ()
	{
		...
		if (currentWaypoint >= racePoints.Count - 1) {
			...
			foreach (BasePlayer item in obj) {
				item.ShowToast (GameTip.Styles.Blue_Normal, raceCompletePhrase, num.ToString (), raceResults.totalParticipants.ToString (), MathEx.SnapTo (startTime, 0.1f).ToString ());
			}
			Kill ();
		} else {
			foreach (BasePlayer item2 in obj) {
				item2.ShowToast (GameTip.Styles.Blue_Normal, stageNotifyPhrase, currentWaypoint.ToString (), (racePoints.Count - 1).ToString (), MathEx.SnapTo (startTime, 0.1f).ToString ());
			}
			SendNetworkUpdate ();
		}
		Pool.FreeList (ref obj);
