...
public class ServerMgr : SingletonComponent<ServerMgr>, IServerCallback
{
	...
	private void OnGiveUserInformation (Message packet)
	{
		...
		if (branch != string.Empty && branch != text) {
			...
			DebugEx.Log (string.Concat ("Kicking ", packet.connection, " - their protocol is ", packet.connection.protocol, " not ", 2403));
	...
	private void UpdateServerInformation ()
	{
		...
		using (TimeWarning.New ("UpdateServerInformation")) {
			...
			SteamServer.GameTags = $"mp{ConVar.Server.maxplayers},cp{BasePlayer.activePlayerList.Count},pt{Network.Net.sv.ProtocolId},qp{SingletonComponent<ServerMgr>.Instance.connectionQueue.Queued},v{2403}{text4}{text6},h{AssemblyHash},{text},{text2},{text3},cs{text7}";
	...
	public void JoinGame (Network.Connection connection)
	{
		using (Approval approval = Facepunch.Pool.Get<Approval> ()) {
			...
			approval.nexusId = NexusServer.NexusId ?? 0;
			NetWrite netWrite = Network.Net.sv.StartWrite ();
