...
public class NavMeshSurface : MonoBehaviour
{
	...
	{
		BuildNavMesh (CollectSources (), calculateWorldBounds: true, GetBuildSettings ());
	}
	...
	public void BuildNavMesh (List<NavMeshBuildSource> sources, bool calculateWorldBounds, NavMeshBuildSettings buildSettings)
	{
		...
		if (calculateWorldBounds && (m_CollectObjects == CollectObjects.All || m_CollectObjects == CollectObjects.Children)) {
			localBounds = CalculateWorldBounds (sources);
		}
		NavMeshData navMeshData = NavMeshBuilder.BuildNavMeshData (buildSettings, sources, localBounds, base.transform.position, base.transform.rotation);
		if (navMeshData != null) {
	...
	{
		return UpdateNavMesh (CollectSources (), calculateWorldBounds: true);
	}

	public AsyncOperation UpdateNavMesh (List<NavMeshBuildSource> sources, bool calculateWorldBounds)
	{
		Bounds localBounds = new Bounds (base.transform.position + m_Center, Abs (m_Size));
		if (calculateWorldBounds && (m_CollectObjects == CollectObjects.All || m_CollectObjects == CollectObjects.Children)) {
			localBounds = CalculateWorldBounds (sources);
		}
		if (m_NavMeshData == null) {
			m_NavMeshData = new NavMeshData (GetBuildSettings ().agentTypeID);
			m_NavMeshDataInstance = NavMesh.AddNavMeshData (m_NavMeshData);
		}
		return NavMeshBuilder.UpdateNavMeshDataAsync (m_NavMeshData, GetBuildSettings (), sources, localBounds);
	}
	...
	{
		if (UpdateOnTransformChanged && HasTransformChanged ()) {
			RemoveData ();
			AddData ();
		}
	}
