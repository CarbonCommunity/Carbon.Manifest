...
public class TOD_Sky : MonoBehaviour
{
	public class ReflectionProbeState
	{
		...
		public GameObject ProbeInstance = null;
	...
	{
		return ReflectionDst.ProbeRenderID > 0 && !ReflectionDst.Probe.IsFinishedRendering (ReflectionDst.ProbeRenderID);
	}
	...
	public void UpdateFog ()
	{
		...
		case TOD_FogType.Atmosphere: {
			Color fogColor2 = SampleFogColor (directLight: false);
			RenderSettings.fogColor = fogColor2;
			break;
		}
		case TOD_FogType.Directional: {
			Color fogColor = SampleFogColor ();
	...
	private void UpdateCelestials ()
	{
		...
		Quaternion quaternion = Quaternion.Euler (90f - World.Latitude, 0f, 0f) * Quaternion.Euler (0f, 180f + num65 * 57.29578f, 0f);
		...
		bool flag2 = Components.SunTransform.localPosition.y > 0f - num123;
		Components.SunRenderer.enabled = flag2;
		bool flag3 = Components.MoonTransform.localPosition.y > 0f - num124;
		...
		if (!Application.isPlaying || timeSinceLightUpdate >= Light.UpdateInterval) {
			...
			Vector3 localPosition3 = (IsNight ? OrbitalToLocal (Mathf.Min (num120, (1f - Light.MinimumHeight) * (float)Math.PI / 2f), num122) : OrbitalToLocal (Mathf.Min (num75, (1f - Light.MinimumHeight) * (float)Math.PI / 2f), num77));
