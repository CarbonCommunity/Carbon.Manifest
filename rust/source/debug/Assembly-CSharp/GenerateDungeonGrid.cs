...
public class GenerateDungeonGrid : ProceduralComponent
{
	...
	public override void Process (uint seed)
	{
			...
			foreach (TerrainPathConnect terrainPathConnect in componentsInChildren) {
				...
				foreach (Prefab<DungeonGridCell> prefab6 in array6) {
					...
					if (!bounds.Intersects2D (bounds2)) {
						...
						Vector3 vector = worldSpaceGrid.GridToWorldCoords (new Vector2i (cellPos.x, cellPos.y));
						Vector3 vector2 = vector + componentInChildren3.UpSocket.localPosition;
						float num4 = (terrainPathConnect.transform.position - vector2).Magnitude2D ();
						if (!(num3 < num4)) {
							prefab5 = prefab6;
							num3 = num4;
						}
		...
		Vector3 vector3 = Vector3.up * 10f;
		Vector3 vector4 = Vector3.up * (LinkTransition + 1f);
		do {
			zero3 = zero2;
			for (int num7 = 0; num7 < worldSpaceGrid.CellCount; num7++) {
				for (int num8 = 0; num8 < worldSpaceGrid.CellCount; num8++) {
					Prefab<DungeonGridCell> prefab12 = worldSpaceGrid [num7, num8];
					if (prefab12 != null) {
						Vector2i cellPos2 = new Vector2i (num7, num8);
						Vector3 vector5 = worldSpaceGrid.GridToWorldCoords (cellPos2);
						while (!prefab12.CheckEnvironmentVolumesInsideTerrain (zero2 + vector5 + vector3, Quaternion.identity, Vector3.one, EnvironmentType.Underground) || prefab12.CheckEnvironmentVolumes (zero2 + vector5 + vector4, Quaternion.identity, Vector3.one, EnvironmentType.Underground | EnvironmentType.Building) || prefab12.CheckEnvironmentVolumes (zero2 + vector5, Quaternion.identity, Vector3.one, EnvironmentType.Underground | EnvironmentType.Building)) {
							zero2.y -= 9f;
						}
					}
				}
		...
		for (int num9 = 0; num9 < worldSpaceGrid.CellCount; num9++) {
			for (int num10 = 0; num10 < worldSpaceGrid.CellCount; num10++) {
				Prefab<DungeonGridCell> prefab13 = worldSpaceGrid [num9, num10];
				if (prefab13 != null) {
					Vector2i cellPos3 = new Vector2i (num9, num10);
					Vector3 vector6 = worldSpaceGrid.GridToWorldCoords (cellPos3);
					World.AddPrefab ("Dungeon", prefab13, zero2 + vector6, Quaternion.identity, Vector3.one);
				}
			}
		}
		for (int num11 = 0; num11 < worldSpaceGrid.CellCount - 1; num11++) {
			for (int num12 = 0; num12 < worldSpaceGrid.CellCount - 1; num12++) {
				Prefab<DungeonGridCell> prefab14 = worldSpaceGrid [num11, num12];
				Prefab<DungeonGridCell> prefab15 = worldSpaceGrid [num11 + 1, num12];
				Prefab<DungeonGridCell> prefab16 = worldSpaceGrid [num11, num12 + 1];
				if (prefab14 != null && prefab15 != null && prefab14.Component.EastVariant != prefab15.Component.WestVariant) {
					...
					foreach (Prefab<DungeonGridCell> prefab17 in array8) {
						if (prefab17.Component.West == prefab14.Component.East && prefab17.Component.East == prefab15.Component.West && prefab17.Component.WestVariant == prefab14.Component.EastVariant && prefab17.Component.EastVariant == prefab15.Component.WestVariant) {
							Vector2i cellPos4 = new Vector2i (num11, num12);
							Vector3 vector7 = worldSpaceGrid.GridToWorldCoords (cellPos4) + new Vector3 (worldSpaceGrid.CellSizeHalf, 0f, 0f);
							World.AddPrefab ("Dungeon", prefab17, zero2 + vector7, Quaternion.identity, Vector3.one);
							break;
						}
					}
				}
				...
				foreach (Prefab<DungeonGridCell> prefab18 in array9) {
					if (prefab18.Component.South == prefab14.Component.North && prefab18.Component.North == prefab16.Component.South && prefab18.Component.SouthVariant == prefab14.Component.NorthVariant && prefab18.Component.NorthVariant == prefab16.Component.SouthVariant) {
						Vector2i cellPos5 = new Vector2i (num11, num12);
						Vector3 vector8 = worldSpaceGrid.GridToWorldCoords (cellPos5) + new Vector3 (0f, 0f, worldSpaceGrid.CellSizeHalf);
						World.AddPrefab ("Dungeon", prefab18, zero2 + vector8, Quaternion.identity, Vector3.one);
						break;
					}
				}
			}
		...
		foreach (PathLink item5 in list3) {
			Vector3 vector9 = item5.upwards.origin.position + item5.upwards.origin.rotation * Vector3.Scale (item5.upwards.origin.upSocket.localPosition, item5.upwards.origin.scale);
			Vector3 vector10 = item5.downwards.origin.position + item5.downwards.origin.rotation * Vector3.Scale (item5.downwards.origin.downSocket.localPosition, item5.downwards.origin.scale);
			Vector3 vector11 = vector10 - vector9;
			Vector3[] array10 = new Vector3[2] {
				new Vector3 (0f, 1f, 0f),
				new Vector3 (1f, 1f, 1f)
			};
			Vector3[] array11 = array10;
			for (int num15 = 0; num15 < array11.Length; num15++) {
				Vector3 b = array11 [num15];
				int num16 = 0;
				int num17 = 0;
				while (vector11.magnitude > 1f && (num16 < 8 || num17 < 8)) {
					...
					Vector3 vector13 = prevSegment.position + prevSegment.rotation * Vector3.Scale (prevSegment.scale, prevSegment.downSocket.localPosition);
					...
					foreach (Prefab<DungeonGridLink> prefab20 in array12) {
						...
						if (component.Rotation > 0) {
							float num22 = Quaternion.Angle (quaternion4, quaternion3);
						...
						Vector3 vector14 = vector13 - quaternion2 * component.UpSocket.localPosition;
						Vector3 vector15 = quaternion2 * (component.DownSocket.localPosition - component.UpSocket.localPosition);
						Vector3 a = vector11 + vector12;
						Vector3 a2 = vector11 + vector15;
						float magnitude = a.magnitude;
						float magnitude2 = a2.magnitude;
						Vector3 vector16 = Vector3.Scale (a, b);
						Vector3 vector17 = Vector3.Scale (a2, b);
						float magnitude3 = vector16.magnitude;
						float magnitude4 = vector17.magnitude;
						...
						if (Mathf.Abs (vector17.x) - Mathf.Abs (vector16.x) > 0.01f || (Mathf.Abs (vector17.x) > 0.01f && a.x * a2.x < 0f) || Mathf.Abs (vector17.y) - Mathf.Abs (vector16.y) > 0.01f || (Mathf.Abs (vector17.y) > 0.01f && a.y * a2.y < 0f) || Mathf.Abs (vector17.z) - Mathf.Abs (vector16.z) > 0.01f || (Mathf.Abs (vector17.z) > 0.01f && a.z * a2.z < 0f) || (flag3 && b.x == 0f && b.z == 0f && component.DownType == DungeonGridLinkType.Default && ((Mathf.Abs (a2.x) > 0.01f && Mathf.Abs (a2.x) < LinkRadius * 2f - 0.1f) || (Mathf.Abs (a2.z) > 0.01f && Mathf.Abs (a2.z) < LinkRadius * 2f - 0.1f)))) {
							continue;
						}
						num18 = num21;
						if (b.x == 0f && b.z == 0f) {
							if (!flag3 && Mathf.Abs (a2.y) < LinkTransition - 0.1f) {
								continue;
							}
						} else if ((!flag3 && magnitude4 > 0.01f && (Mathf.Abs (a2.x) < LinkRadius * 2f - 0.1f || Mathf.Abs (a2.z) < LinkRadius * 2f - 0.1f)) || (!flag4 && magnitude4 > 0.01f && (Mathf.Abs (a2.x) < LinkRadius * 1f - 0.1f || Mathf.Abs (a2.z) < LinkRadius * 1f - 0.1f))) {
							continue;
						}
						if (!flag3 || !(magnitude4 < 0.01f) || !(magnitude2 < 0.01f) || !(Quaternion.Angle (quaternion4, quaternion3) > 10f)) {
							prefab19 = prefab20;
							vector12 = vector15;
							num18 = num21;
							position3 = vector14;
					...
					if (b.x > 0f || b.z > 0f) {
						...
						Vector3 vector19 = prevSegment3.position + prevSegment3.rotation * Vector3.Scale (prevSegment3.scale, prevSegment3.upSocket.localPosition);
						...
						foreach (Prefab<DungeonGridLink> prefab22 in array13) {
							...
							if (component2.Rotation > 0) {
								float num27 = Quaternion.Angle (quaternion9, quaternion8);
							...
							Vector3 vector20 = vector19 - quaternion7 * component2.DownSocket.localPosition;
							Vector3 vector21 = quaternion7 * (component2.UpSocket.localPosition - component2.DownSocket.localPosition);
							Vector3 a3 = vector11 - vector18;
							Vector3 a4 = vector11 - vector21;
							float magnitude5 = a3.magnitude;
							float magnitude6 = a4.magnitude;
							Vector3 vector22 = Vector3.Scale (a3, b);
							Vector3 vector23 = Vector3.Scale (a4, b);
							float magnitude7 = vector22.magnitude;
							float magnitude8 = vector23.magnitude;
							if (vector18 != Vector3.zero) {
							...
							if (Mathf.Abs (vector23.x) - Mathf.Abs (vector22.x) > 0.01f || (Mathf.Abs (vector23.x) > 0.01f && a3.x * a4.x < 0f) || Mathf.Abs (vector23.y) - Mathf.Abs (vector22.y) > 0.01f || (Mathf.Abs (vector23.y) > 0.01f && a3.y * a4.y < 0f) || Mathf.Abs (vector23.z) - Mathf.Abs (vector22.z) > 0.01f || (Mathf.Abs (vector23.z) > 0.01f && a3.z * a4.z < 0f) || (flag3 && b.x == 0f && b.z == 0f && component2.UpType == DungeonGridLinkType.Default && ((Mathf.Abs (a4.x) > 0.01f && Mathf.Abs (a4.x) < LinkRadius * 2f - 0.1f) || (Mathf.Abs (a4.z) > 0.01f && Mathf.Abs (a4.z) < LinkRadius * 2f - 0.1f)))) {
								continue;
							}
							num23 = num26;
							if (b.x == 0f && b.z == 0f) {
								if (!flag3 && Mathf.Abs (a4.y) < LinkTransition - 0.1f) {
									continue;
								}
							} else if ((!flag3 && magnitude8 > 0.01f && (Mathf.Abs (a4.x) < LinkRadius * 2f - 0.1f || Mathf.Abs (a4.z) < LinkRadius * 2f - 0.1f)) || (!flag4 && magnitude8 > 0.01f && (Mathf.Abs (a4.x) < LinkRadius * 1f - 0.1f || Mathf.Abs (a4.z) < LinkRadius * 1f - 0.1f))) {
								continue;
							}
							if (!flag3 || !(magnitude8 < 0.01f) || !(magnitude6 < 0.01f) || !(Quaternion.Angle (quaternion9, quaternion8) > 10f)) {
								prefab21 = prefab22;
								vector18 = vector21;
								num23 = num26;
								position4 = vector20;
