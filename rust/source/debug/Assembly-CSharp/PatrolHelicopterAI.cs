...
public class PatrolHelicopterAI : BaseMonoBehaviour
{
	...
	public bool PlayerVisible (BasePlayer ply)
	{
		...
		if (ply.eyes.position.y < WaterSystem.OceanLevel) {
			float f = WaterSystem.OceanLevel - ply.eyes.position.y;
			if (Mathf.Abs (f) > oceanDepthTargetCutoff) {
				return false;
			}
		}
	...
	public void TerrainPushback ()
	{
		if (_currentState != aiState.DEATH) {
			...
			if (num8 < num5) {
				...
				float num10 = terrainPushForce * num9;
				b2 = Vector3.up * num10;
			}
	...
	public void WeakspotDamaged (BaseHelicopter.weakspot weak, HitInfo info)
	{
		...
		if (num < 5f && basePlayer != null && (flag2 || flag3)) {
			ExitCurrentState ();
			State_Strafe_Enter (info.Initiator.transform.position, flag3);
		}
	}
	...
	public void FireGun (Vector3 targetPos, float aimCone, bool left)
	{
		...
		Transform transform = (left ? helicopterBase.left_gun_muzzle.transform : helicopterBase.right_gun_muzzle.transform);
				...
				if ((bool)entity && entity != helicopterBase) {
					...
					if ((bool)baseCombatEntity) {
						...
						if (baseCombatEntity is BasePlayer) {
							...
							hitInfo2.HitPositionWorld = hitInfo.point - modifiedAimConeDirection * 0.25f;
	...
	public void FireRocket ()
	{
		...
		if (UnityEngine.Physics.Raycast (vector, vector2, out var hitInfo, maxDistance, 1237003025)) {
