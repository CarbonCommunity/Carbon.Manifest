...
public class PidQuaternionController
{
	...
	public Vector3 ComputeRequiredAngularAcceleration (Quaternion currentOrientation, Quaternion desiredOrientation, Vector3 currentAngularVelocity, float deltaTime)
	{
		...
		Quaternion error = Quaternion.identity.Subtract (quaternion);
		Quaternion quaternion2 = ToEulerAngleQuaternion (currentAngularVelocity);
		Quaternion delta = quaternion2 * quaternion;
		...
		Quaternion quaternion3 = ComputeOutput (error, delta, deltaTime);
		quaternion3 = MultiplyAsVector (matrix, quaternion3);
		Quaternion quaternion4 = quaternion3.Multiply (-2f);
		Quaternion quaternion5 = quaternion4 * Quaternion.Inverse (quaternion);
