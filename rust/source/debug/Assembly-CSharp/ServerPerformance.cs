...
public class ServerPerformance : BaseMonoBehaviour
{
	...
	public static void ComponentReport (string filename, string Title, UnityEngine.Object[] objects)
	{
		...
		IEnumerable<IGrouping<string, UnityEngine.Object>> source = from x in objects
			group x by WorkoutPrefabName ((x as Component).gameObject);
		source = source.OrderByDescending ((IGrouping<string, UnityEngine.Object> x) => x.Count ());
		foreach (IGrouping<string, UnityEngine.Object> item in source) {
