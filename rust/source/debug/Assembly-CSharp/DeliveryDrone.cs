...
public class DeliveryDrone : Drone
{
	...
	private void Think ()
	{
		...
		{
			Vector3 value = body.velocity.WithY (0f);
			value.ToDirectionAndMagnitude (out var direction, out var magnitude);
			if (magnitude < 0.5f) {
				float num4 = GetMinimumHeight (Vector3.zero) + preferredCruiseHeight;
				Vector3 origin = currentPosition.WithY (num4 + 1000f);
				Vector3 vector2 = currentPosition.WithY (num4);
				isBlocked = Physics.Raycast (origin, Vector3.down, out var hitInfo, 1000f, config.layerMask);
				return isBlocked ? (num4 + (1000f - hitInfo.distance) + preferredHeightAboveObstacle) : num4;
			}
			...
			float minimumHeight3 = GetMinimumHeight (new Vector3 (0f, 0f, num5));
			float num6 = Mathf.Max (Mathf.Max (minimumHeight, minimumHeight2), minimumHeight3) + preferredCruiseHeight;
			...
			Vector3 v = currentPosition.WithY (num6) + quaternion * new Vector3 (0f, 0f, halfExtents.z / 2f);
			Vector3 vector3 = v.WithY (num6 + 1000f);
			isBlocked = Physics.BoxCast (vector3, halfExtents, Vector3.down, out var hitInfo2, quaternion, 1000f, config.layerMask);
			float result;
			if (isBlocked) {
				Ray ray = new Ray (vector3, Vector3.down);
				Vector3 b = ray.ClosestPoint (hitInfo2.point);
				float num7 = Vector3.Distance (ray.origin, b);
				result = num6 + (1000f - num7) + preferredHeightAboveObstacle;
			} else {
				result = num6;
			}
			return result;
		...
		{
			Vector3 vector4 = base.transform.TransformPoint (offset);
			float height = TerrainMeta.HeightMap.GetHeight (vector4);
			float height2 = WaterSystem.GetHeight (vector4);
