...
public class FishingBobber : BaseCombatEntity
{
	...
	{
		float num = Vector3.Angle ((pos - playerPos).normalized.WithY (0f), initialDirection);
		if (num > 60f) {
			return false;
		}
		Vector3 position = base.transform.position;
		Ray ray = new Ray (position, (pos - position).normalized);
		if (GamePhysics.Trace (ray, 0.1f, out var _, checkLength, 1218511105)) {
			return false;
		}
		return true;
	}
