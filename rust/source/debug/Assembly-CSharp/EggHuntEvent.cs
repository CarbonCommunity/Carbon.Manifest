...
public class EggHuntEvent : BaseHuntEvent
{
	...
	public void EggCollected (BasePlayer player)
	{
		...
		int num2 = ((!((float)num * 0.5f < 1f)) ? 1 : UnityEngine.Random.Range (0, 2));
		SpawnEggsAtPoint (UnityEngine.Random.Range (1 + num2, 2 + num2), player.transform.position, player.eyes.BodyForward (), 15f, 25f);
	}
