...
public class ZiplineMountable : BaseMountable
{
	...
	private Vector3 GetLineEndPoint (bool applyDismountOffset = false)
	{
		if (applyDismountOffset && linePoints != null) {
			...
			Vector3 vector = linePoints [linePoints.Count - 1];
			return vector + normalized * 1.5f;
		}
	...
	public void SetDestination (List<Vector3> targetLinePoints, Vector3 lineStartPos, Quaternion lineStartRot)
	{
		...
		GamePhysics.OverlapSphere (position, 6f, ignoreColliders, 1218511105);
	...
	private void Update ()
	{
		...
		float num = (MoveSpeed + additiveValue * ForwardAdditive) * Mathf.Clamp ((float)mountTime / SpeedUpTime, 0f, 1f);
		float num2 = num * UnityEngine.Time.smoothDeltaTime;
		...
		GamePhysics.CapsuleSweep (position, position2, ZipCollider.radius, base.transform.forward, num2, obj, 1218511105);
