...
public class FishShoal : IDisposable
{
	...
	public void TrySpawn (float3 cameraPosition)
	{
		...
		int num3 = Mathf.CeilToInt (fishType.maxCount * GetPopulationScaleForPoint (cameraPosition));
		int num4 = num3 - num2;
		if (num4 <= 0) {
			return;
		}
		...
		for (int i = num6; i < num7; i++) {
			Unity.Mathematics.Random random = new Unity.Mathematics.Random ((uint)(i * 3245 + num5));
