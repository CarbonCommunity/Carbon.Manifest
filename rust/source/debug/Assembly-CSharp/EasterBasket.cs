...
public class EasterBasket : AttackEntity
{
	...
	public void ThrowEgg (RPCMessage msg)
	{
		...
		if (!VerifyClientAttack (player)) {
			...
			float num2 = 1f;
			if (UnityEngine.Physics.Raycast (vector, vector2, out var hitInfo, num2, 1237003025)) {
				num2 = hitInfo.distance - 0.1f;
			}
			BaseEntity baseEntity = GameManager.server.CreateEntity (eggProjectile.resourcePath, vector + vector2 * num2);
			if (!(baseEntity == null)) {
