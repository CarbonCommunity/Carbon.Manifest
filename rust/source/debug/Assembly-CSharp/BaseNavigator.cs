...
public class BaseNavigator : BaseMonoBehaviour
{
	...
	[InspectorFlags]
	public TerrainTopology.Enum topologyPrevent = (TerrainTopology.Enum)0;

	[InspectorFlags]
	public TerrainBiome.Enum biomeRequirement = (TerrainBiome.Enum)0;
	...
	public bool SetDestination (Vector3 pos, float speedFraction = 1f, float updateInterval = 0f, float navmeshSampleDistance = 0f)
	{
		...
		default: {
			...
			if (AI.usecalculatepath) {
				flag2 = NavMesh.CalculatePath (base.transform.position, Destination, navMeshQueryFilter, path);
				if (flag2) {
					Agent.SetPath (path);
