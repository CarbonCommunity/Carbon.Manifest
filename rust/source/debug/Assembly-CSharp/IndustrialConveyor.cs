...
public class IndustrialConveyor : IndustrialEntity
{
	...
	public struct ItemFilter
	{
		...
		public string TargetItemName {
			get {
				return (TargetItem != null) ? TargetItem.shortname : string.Empty;
			}
			set {
	...
	private Item GetItemToMove (IIndustrialStorage storage, out ItemFilter associatedFilter, int slot, ItemContainer targetContainer = null)
	{
		...
		for (int i = vector2i.x; i <= vector2i.y; i++) {
			...
			if (targetContainer != null && associatedFilter.TargetItem != null && associatedFilter.MaxAmountInOutput > 0) {
				int totalItemAmount = targetContainer.GetTotalItemAmount (slot2, vector2i.x, vector2i.y);
			...
			foreach (ContainerInputOutput splitOutput in splitOutputs) {
				...
				foreach (ContainerInputOutput splitInput in splitInputs) {
					...
					for (int i = vector2i.x; i <= vector2i.y; i++) {
						...
						if (flag3 && filter2.Item1.TargetItem != null && filter2.Item1.MaxAmountInOutput > 0) {
							int totalItemAmount = splitOutput.Storage.Container.GetTotalItemAmount (slot, range.x, range.y);
						...
						if (flag3 && filter2.Item1.MinAmountInInput > 0) {
							if (filter2.Item1.TargetItem != null && FilterMatchItem (filter2.Item1, slot)) {
								int totalItemAmount2 = container.GetTotalItemAmount (slot, vector2i.x, vector2i.y);
