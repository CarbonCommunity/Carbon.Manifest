...
public class LargeShredder : BaseEntity
{
	...
	public void CreateShredResources ()
	{
		...
		foreach (ItemAmount itemAmount in shredResources) {
			...
			float num = 0.5f;
			BaseEntity baseEntity = item.CreateWorldObject (resourceSpawnPoint.transform.position + new Vector3 (Random.Range (0f - num, num), 1f, Random.Range (0f - num, num)));
			if (baseEntity == null) {
				item.Remove ();
			}
		}
		...
		foreach (BaseVehicleModule attachedModuleEntity in component2.AttachedModuleEntities) {
			...
			foreach (ItemAmount ingredient in attachedModuleEntity.AssociatedItemDef.Blueprint.ingredients) {
				...
				if (num2 != 0) {
					...
					float num3 = 0.5f;
					BaseEntity baseEntity2 = item2.CreateWorldObject (resourceSpawnPoint.transform.position + new Vector3 (Random.Range (0f - num3, num3), 1f, Random.Range (0f - num3, num3)));
					if (baseEntity2 == null) {
						item2.Remove ();
					}
				}
