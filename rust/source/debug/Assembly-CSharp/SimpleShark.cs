...
public class SimpleShark : BaseCombatEntity
{
	...
	public class IdleState : SimpleState
	{
		...
		public override void State_Enter ()
		{
			...
			for (int i = 0; i < entity.patrolPath.Count; i++) {
				Vector3 a = entity.patrolPath [i];
				float num3 = Vector3.Distance (a, entity.transform.position);
				if (num3 < num) {
					num2 = i;
					num = num3;
				}
		...
		public override void State_Think (float delta)
		{
			...
			if (Vector3.Distance (targetPatrolPosition, entity.transform.position) < entity.stoppingDistance) {
				patrolTargetIndex++;
	...
	public void UpdateTarget (float delta)
	{
		...
		if (target != null) {
			float num = Vector3.Distance (target.eyes.position, base.transform.position);
	...
	private void UpdateDirection (float delta)
	{
		...
		Vector3 forward = base.transform.forward;
		Vector3 aimAt = WaterClamp (destination);
		Vector3 vector = Vector3Ex.Direction (aimAt, base.transform.position);
		if (obstacleAvoidanceScale != 0f) {
			Vector3 vector2;
