...
public class OreResourceEntity : StagedResourceEntity
{
	...
	public override void OnAttacked (HitInfo info)
	{
		...
		if (!info.DidGather && info.gatherScale > 0f) {
			...
			if ((bool)_hotSpot || (bool)jackhammer) {
				...
				float num = Vector3.Distance (info.HitPositionWorld, _hotSpot.transform.position);
				if (num <= _hotSpot.GetComponent<SphereCollider> ().radius * 1.5f || jackhammer != null) {
	...
	public OreHotSpot SpawnBonusSpot (Vector3 lastDirection)
	{
		...
		bonusesSpawned++;
		Vector3 normalized2 = (stageComponent.bounds.center - zero).normalized;
		if (stageComponent.Raycast (new Ray (zero, normalized2), out var hitInfo, 10f)) {
			OreHotSpot oreHotSpot = GameManager.server.CreateEntity (bonusPrefab.resourcePath, hitInfo.point - normalized2 * 0.025f, Quaternion.LookRotation (hitInfo.normal, Vector3.up)) as OreHotSpot;
			oreHotSpot.Spawn ();
