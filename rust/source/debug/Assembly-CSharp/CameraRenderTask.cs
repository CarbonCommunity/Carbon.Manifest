...
public class CameraRenderTask : CustomYieldInstruction, IDisposable
{
	...
	public override bool keepWaiting => _pendingJob.HasValue && !_pendingJob.Value.IsCompleted;
	...
	public int Start (int width, int height, float verticalFov, float nearPlane, float farPlane, int layerMask, Transform cameraTransform, int sampleCount, int sampleOffset, Dictionary<int, (byte MaterialIndex, int Age)> knownColliders)
	{
		...
		JobHandle dependsOn = jobData2.Schedule (sampleCount, 100);
		JobHandle job2 = RaycastCommand.ScheduleBatch (_raycastCommands.GetSubArray (0, sampleCount), _raycastHits.GetSubArray (0, sampleCount), 100, dependsOn);
	...
	private static NativeArray<int2> GetSamplePositions (int width, int height)
	{
		...
		raycastSamplePositionsJob.random = new Unity.Mathematics.Random (1337u);
		raycastSamplePositionsJob.positions = value;
		RaycastSamplePositionsJob jobData = raycastSamplePositionsJob;
		jobData.Run ();
