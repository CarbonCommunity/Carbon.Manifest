...
public class BaseVehicle : BaseMountable
{
	...
	public override void VehicleFixedUpdate ()
	{
		...
		if (clippingChecks != 0 && AnyMounted ()) {
			...
			Collider[] array = UnityEngine.Physics.OverlapBox (center, bounds.extents, base.transform.rotation, GetClipCheckMask ());
			if (array.Length != 0) {
	...
	public virtual bool IsSeatVisible (BaseMountable mountable, Vector3 eyePos, int mask = 1218511105)
	{
		...
		Vector3 p = position + base.transform.up * 0.15f;
	...
	{
		return spawnTime == -1f || spawnTime + 300f < UnityEngine.Time.realtimeSinceStartup;
	}
	...
	public bool IsSafe ()
	{
		return OnlyOwnerAccessible () && Vector3.Distance (safeAreaOrigin, base.transform.position) <= safeAreaRadius;
	}
	...
	private BaseMountable SpawnMountPoint (MountPointInfo mountToSpawn, Model model)
	{
		...
		if (mountToSpawn.bone != "") {
			Transform transform = model.FindBone (mountToSpawn.bone);
			Vector3 position = transform.transform.position;
			position += base.transform.TransformDirection (mountToSpawn.pos);
			pos = position;
	...
	protected virtual void DoPushAction (BasePlayer player)
	{
		...
		if (IsFlipped ()) {
			...
			Vector3 lhs = base.transform.InverseTransformVector (base.transform.position - player.transform.position);
			...
			Vector3 vector = base.transform.position - player.eyes.position;
			Vector3 normalized = Vector3.ProjectOnPlane (vector, base.transform.up).normalized;
	...
	{
		return IsAlive () && !base.IsDestroyed && player != null;
	}
