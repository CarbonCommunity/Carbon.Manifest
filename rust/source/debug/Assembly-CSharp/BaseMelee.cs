			...
			if (ConVar.AntiHack.melee_protection > 0) {
				...
				if (ConVar.AntiHack.melee_protection >= 3) {
					if (flag6) {
						...
						bool flag9 = GamePhysics.LineOfSight (center - vector2, position + vector2, num5) && GamePhysics.LineOfSight (position - vector3, pointStart + vector3, num5) && GamePhysics.LineOfSight (pointStart - vector4, vector, num5) && GamePhysics.LineOfSight (vector, hitPositionWorld, num5);
						if (!flag9) {
						...
						if (!flag9) {
							...
							AntiHack.Log (player, AntiHackType.MeleeHack, string.Concat ("Line of sight (", shortPrefabName10, " on ", shortPrefabName11, ") ", center, " ", position, " ", pointStart, " ", vector, " ", hitPositionWorld));
					...
					if (flag8 && flag && !flag7) {
						...
						if (!flag10) {
							flag10 = GamePhysics.LineOfSight (hitPositionWorld2, vector5, num5, 0f, melee_losforgiveness) && GamePhysics.LineOfSight (vector5, hitPositionWorld2, num5, melee_losforgiveness, 0f);
						}
						if (!flag10) {
							string shortPrefabName12 = base.ShortPrefabName;
							string shortPrefabName13 = basePlayer.ShortPrefabName;
							AntiHack.Log (player, AntiHackType.MeleeHack, string.Concat ("Line of sight (", shortPrefabName12, " on ", shortPrefabName13, ") ", hitPositionWorld2, " ", position2, " or ", hitPositionWorld2, " ", vector5));
							player.stats.combat.LogInvalid (hitInfo, "melee_los");
							flag8 = false;
						}
					}
