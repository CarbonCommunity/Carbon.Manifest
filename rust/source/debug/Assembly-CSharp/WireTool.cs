...
public class WireTool : HeldEntity
{
	...
	{
		return pending.ent != null && pending.index != -1 && ((pending.input && pending.ent.inputs [pending.index].type == type) || (!pending.input && pending.ent.outputs [pending.index].type == type));
	}
	...
	{
		return pending.ent != null && pending.ent.IsRootEntity ();
	}
	...
	public void MakeConnection (RPCMessage msg)
	{
		...
		if (!(iOEntity2 == null) && ValidateLine (list, iOEntity, iOEntity2, player, num3) && !(Vector3.Distance (baseNetworkable2.transform.position, baseNetworkable.transform.position) > maxWireLength) && num2 < iOEntity.inputs.Length && num3 < iOEntity2.outputs.Length && !(iOEntity.inputs [num2].connectedTo.Get () != null) && !(iOEntity2.outputs [num3].connectedTo.Get () != null) && (!iOEntity.inputs [num2].rootConnectionsOnly || iOEntity2.IsRootEntity ()) && CanModifyEntity (player, iOEntity) && CanModifyEntity (player, iOEntity2)) {
	...
	public static void AttemptClearSlot (BaseNetworkable clearEnt, BasePlayer ply, int clearIndex, bool isInput)
	{
		...
		if (isInput && iOEntity2 != null) {
			...
			foreach (IOEntity.IOSlot iOSlot3 in inputs) {
				if (iOSlot3.mainPowerSlot && (bool)iOSlot3.connectedTo.Get ()) {
