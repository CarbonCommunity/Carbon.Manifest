...
public class DiagnosticsConSys : ConsoleSystem
{
	...
	private static void DumpObjects (string targetFolder)
	{
		...
		stringBuilder3.AppendLine ();
		foreach (IGrouping<Type, UnityEngine.Object> item3 in from x in source
			group x by x.GetType () into x
			orderby x.Sum ((UnityEngine.Object y) => Profiler.GetRuntimeMemorySize (y)) descending
			select x) {
			stringBuilder3.AppendFormat ("{2}{1}{0}", item3.First ().GetType ().Name, item3.Count ().ToString ("N0").PadRight (12), item3.Sum ((UnityEngine.Object y) => Profiler.GetRuntimeMemorySize (y)).FormatBytes ().PadRight (20));
			stringBuilder3.AppendLine ();
		}
