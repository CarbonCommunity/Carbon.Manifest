...
public class DungeonNavmesh : FacepunchBehaviour, IServerComponent
{
	...
	public void SourcesCollected ()
	{
		...
		for (int num = sources.Count - 1; num >= 0; num--) {
			...
			bool flag = false;
			foreach (AIInformationZone zone in AIInformationZone.zones) {
				Vector3 a = zone.ClosestPointTo (vector);
				float num2 = Vector3Ex.Distance2D (a, vector);
				if (num2 <= 50f) {
					flag = true;
				}
			}
	...
	public IEnumerator UpdateNavMeshAndWait ()
	{
		...
		IEnumerator collectSourcesAsync = NavMeshTools.CollectSourcesAsync (base.transform, LayerMask.value, NavMeshCollectGeometry, defaultArea, sources, AppendModifierVolumes, UpdateNavMeshAsync);
