...
public class ZiplineLaunchPoint : BaseEntity
{
				...
				if (flag && flag2) {
					...
					foreach (ZiplineTarget item3 in list) {
						if (!(item3 == item2) && item3.IsValidChainPoint (item2.Target.position, item.Target.position)) {
							bool flag3 = CheckLineOfSight (item2.transform.position, item3.transform.position);
							bool flag4 = CheckLineOfSight (item3.transform.position, item.transform.position);
							if (flag3 && flag4) {
								num = num4;
								ziplineTarget = item;
								foundPositions.Clear ();
	...
	private bool CheckLineOfSight (Vector3 from, Vector3 to)
	{
		...
		return GamePhysics.LineOfSightRadius (from, to, 1218511105, 0.5f, 2f) && GamePhysics.LineOfSightRadius (from, vector, 1218511105, 0.5f, 2f) && GamePhysics.LineOfSightRadius (vector, to, 1218511105, 0.5f, 2f);
	}
	...
	private void UpdateBuildingBlocks ()
	{
		...
		foreach (BoxCollider boxCollider in buildingBlocks) {
			boxCollider.gameObject.SetActive (value: false);
		}
		BoxCollider[] pointBuildingBlocks = PointBuildingBlocks;
		foreach (BoxCollider boxCollider2 in pointBuildingBlocks) {
			boxCollider2.gameObject.SetActive (value: false);
		}
		...
		foreach (SpawnableBoundsBlocker spawnableBoundsBlocker in spawnableBoundsBlockers) {
			spawnableBoundsBlocker.gameObject.SetActive (value: false);
		}
		int num = 0;
		if (ziplineTargets.Count <= 0) {
			return;
		...
		for (int l = 0; l < linePoints.Count; l++) {
			...
			Vector3 vector2 = linePoints [l];
			Vector3 normalized = (vector2 - linePoints [l - 1].WithY (vector2.y)).normalized;
		...
		void SetUpBuildingBlock (BoxCollider longCollider, BoxCollider pointCollider, SpawnableBoundsBlocker spawnBlocker, int startIndex, int endIndex)
		{
			...
			for (int m = startIndex; m < endIndex; m++) {
				Vector3 vector6 = longCollider.transform.InverseTransformPoint (linePoints [m]);
	...
	{
		float num = Vector3.Dot (ply.eyes.HeadForward (), base.transform.forward);
		return num > 0.2f;
	}
