...
public class RidableHorse : BaseRidableAnimal
{
	...
	public void TryHitch ()
	{
		...
		foreach (HitchTrough item in obj) {
			Vector3 lhs = Vector3Ex.Direction2D (item.transform.position, base.transform.position);
			if (Vector3.Dot (lhs, base.transform.forward) < 0.4f || item.isClient || !item.HasSpace () || !item.ValidHitchPosition (base.transform.position) || !item.AttemptToHitch (this)) {
				continue;
			}
			break;
		}
