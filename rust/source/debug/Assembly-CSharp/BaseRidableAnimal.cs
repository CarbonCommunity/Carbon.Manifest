...
public class BaseRidableAnimal : BaseVehicle
{
	...
	{
		return saddleRef.IsValid (base.isServer) ? saddleRef.Get (base.isServer).GetComponent<BaseMountable> () : null;
	}
	...
	private void DoDung ()
	{
		...
		item.Drop (base.transform.position + -base.transform.forward + Vector3.up * 1.1f + UnityEngine.Random.insideUnitSphere * 0.1f, -base.transform.forward);
	}
	...
	public void UpdateGroundNormal (bool force = false)
	{
		if (UnityEngine.Time.time >= nextGroundNormalUpdateTime || force) {
			...
			foreach (Transform transform in array) {
				...
				if (TransformUtil.GetGroundInfo (position + Vector3.up * 2f, out var _, out var normal, 4f, 295763969)) {
					targetUp += normal;
				} else {
					targetUp += Vector3.up;
				}
			...
			if (flag) {
				...
				if (num8 > maxWallClimbSlope || num9 > maxWallClimbSlope) {
					...
					for (int j = 0; j < normalOffsets.Length; j++) {
						Vector3 startPos = vector4;
						startPos += normalOffsets [j].x * base.transform.right;
						float num12 = maxStepHeight * 2.5f;
						startPos += Vector3.up * num12;
						startPos += normalOffsets [j].z * base.transform.forward;
						if (TransformUtil.GetGroundInfo (startPos, out var pos2, out var normal2, num7 + num12, 295763969)) {
							num11++;
							vector5 += normal2;
							num10 += pos2.y;
						}
					...
					num10 /= (float)num11;
					vector5.Normalize ();
					num8 = Vector3.Angle (up, vector5);
					num9 = Vector3.Angle (vector5, Vector3.up);
					if (num8 > maxWallClimbSlope || num9 > maxWallClimbSlope || Mathf.Abs (num10 - vector4.y) > maxStepHeight) {
						Profiler.EndSample ();
						return num4;
					}
				}
	...
	public void UpdateMovement (float delta)
	{
		...
		float num7 = 1f - Mathf.InverseLerp (20f, 45f, Vector3.Angle (Vector3.up, averagedUp));
		num6 = num5 * 0.1f + num6 * 0.9f;
		float value2 = Mathf.Min (num7 + 0.2f, num6);
		value2 = Mathf.Clamp01 (value2);
		float num8 = Mathf.Min (value2 * GetRunSpeed (), desiredVelocity);
		float num9 = ((num8 < currentSpeed) ? 3f : 1f);
		if (Mathf.Abs (currentSpeed) < 2f && desiredVelocity == 0f) {
			currentSpeed = Mathf.MoveTowards (currentSpeed, 0f, delta * 3f);
		} else {
			currentSpeed = Mathf.Lerp (currentSpeed, num8, delta * num9);
		}
		if (num6 == 0f) {
			currentSpeed = 0f;
		}
		float num10 = 1f - Mathf.InverseLerp (2f, 7f, currentSpeed);
		num10 = (num10 + 1f) / 2f;
		if (desiredRotation != 0f) {
			Vector3 position2 = animalFront.transform.position;
			Quaternion rotation = base.transform.rotation;
			base.transform.Rotate (Vector3.up, desiredRotation * delta * turnSpeed * num10);
		...
		float num11 = currentSpeed * delta;
		Vector3 vector2 = base.transform.position + normalized * num11 * Mathf.Sign (currentSpeed);
		...
		if (currentSpeed > 0f || timeAlive < 2f || (float)dropUntilTime > 0f) {
			...
			RaycastHit hitInfo;
			bool flag3 = UnityEngine.Physics.SphereCast (animalFront.transform.position, obstacleDetectionRadius, normalized, out hitInfo, num11, 1503731969);
			bool flag4 = UnityEngine.Physics.SphereCast (base.transform.position + base.transform.InverseTransformPoint (animalFront.transform.position).y * base.transform.up, obstacleDetectionRadius, normalized, out hitInfo, num11, 1503731969);
			if (!Vis.AnyColliders (animalFront.transform.position + normalized * num11, obstacleDetectionRadius, 1503731969) && !flag3 && !flag4) {
				if (DropToGround (vector2 + Vector3.up * maxStepHeight)) {
					MarkDistanceTravelled (num11);
				} else {
					currentSpeed = 0f;
				}
			} else {
