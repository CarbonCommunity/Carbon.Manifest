...
public class CH47FlightTest : MonoBehaviour
{
	...
	public HelicopterInputState_t GetAIInputState ()
	{
		...
		Vector3 lhs = Vector3.Cross (Vector3.up, vector);
		float num = Vector3.Dot (lhs, Vector3Ex.Direction2D (AIMoveTarget.position, base.transform.position));
	...
	public void FixedUpdate ()
	{
		...
		for (int i = 0; i < GroundEffects.Length; i++) {
			Transform transform = GroundPoints [i];
			Transform transform2 = GroundEffects [i];
			if (Physics.Raycast (transform.transform.position, Vector3.down, out var hitInfo, 50f, 8388608)) {
				transform2.gameObject.SetActive (value: true);
				transform2.transform.position = hitInfo.point + new Vector3 (0f, 1f, 0f);
			} else {
