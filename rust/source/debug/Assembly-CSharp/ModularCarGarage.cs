...
public class ModularCarGarage : ContainerIOEntity
{
	...
	private ModularCar carOccupant => (lockedOccupant != null) ? lockedOccupant : occupantTrigger.carOccupant;

	private bool HasOccupant => carOccupant != null && carOccupant.IsFullySpawned ();
	...
	private void MoveLift (VehicleLiftState desiredLiftState, float startDelay = 0f, bool forced = false)
	{
		if (this.vehicleLiftState != desiredLiftState || forced) {
			...
			if (!base.gameObject.activeSelf) {
				...
				animationState.time = ((desiredLiftState == VehicleLiftState.Up) ? 1f : 0f);
