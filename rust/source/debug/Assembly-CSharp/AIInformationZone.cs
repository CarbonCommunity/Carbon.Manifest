...
public class AIInformationZone : BaseMonoBehaviour, IServerComponent
{
	...
	private bool ProcessDistances ()
	{
		...
		while (PassesBudget (realtimeSinceStartup, budgetSeconds)) {
			...
			foreach (AICoverPoint coverPoint in coverPoints) {
				...
				float num4 = -1f;
				float num5 = Vector3.Distance (aIMovePoint.transform.position, coverPoint.transform.position);
				if (num5 > 40f) {
					num4 = -2f;
				} else {
					Profiler.BeginSample ("AIInformationZoneProcessing-CalculatePath");
					bool flag2 = NavMesh.CalculatePath (aIMovePoint.transform.position, coverPoint.transform.position, areaMask, navMeshPath) && navMeshPath.status == NavMeshPathStatus.PathComplete;
					Profiler.EndSample ();
					if (flag2) {
						int num6 = navMeshPath.corners.Length;
						if (num6 > 1) {
							Vector3 a = navMeshPath.corners [0];
							float num7 = 0f;
							for (int i = 0; i < num6; i++) {
								Vector3 vector = navMeshPath.corners [i];
								num7 += Vector3.Distance (a, vector);
								a = vector;
							}
							num4 = num7;
							pathSuccesses++;
	...
	public AICoverPoint GetBestCoverPoint (Vector3 currentPosition, Vector3 hideFromPosition, float minRange = 0f, float maxRange = 20f, BaseEntity forObject = null, bool allowObjectToReuse = true)
	{
		...
		for (int i = 0; i < pointCount; i++) {
			...
			Vector3 normalized2 = (aICoverPoint2.transform.position - currentPosition).normalized;
			float value2 = Vector3.Dot (normalized2, normalized);
	...
	public NavMeshLink GetClosestNavMeshLink (Vector3 pos)
	{
		...
		foreach (NavMeshLink navMeshLink in navMeshLinks) {
			Vector3 position = navMeshLink.gameObject.transform.position;
